...
| **P2-D7** | v1's SQS publish happens in a Celery worker with **no retry**; a failure is logged and the message lost. Its boto3 client uses botocore's *legacy* retry mode, i.e. **up to 5** HTTP attempts per `send_message`, with no request timeout. | v2 publishes inline with **3 attempts and exponential backoff** (200 ms, 400 ms). ⚠️ **Corrected under condition C22 (review finding S4):** the original wording said "3 attempts" while `@aws-sdk/client-sqs` was still at its own default `maxAttempts: 3`, so the real bound was **3 × 3 = 9** HTTP attempts over an *unbounded* period — on the HTTP request thread, where v1's was in a worker. The SDK is now pinned to `maxAttempts: 1` with a 1 s connect / 2 s socket timeout (`sqs.client.ts`), so the publisher's loop is the only retry and the **total wall-clock bound is 3 × 3 s + 600 ms ≈ 9.6 s worst case**. The same treatment is applied to the SES client (1 attempt, 1 s / 5 s), where boto3's legacy default was 5 and `MailService` has no retry loop of its own. | Plan Phase 2, condition 1 — authorised in advance. Strictly an improvement; the swallow semantics at the boundary are unchanged (condition 2). |
