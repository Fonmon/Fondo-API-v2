...
| Date | Rev | Change |
|---|---|---|
| 2026-09-15 | v4.40 | ✅ **Phase 9 code work COMPLETE at `0bf23da`** — M-D34-8 restated as a two-population divergence in both places, N12 closed, and Release A described as the tag it is everywhere (§6.1 header and gate numbers, §6.7's recovery row, the stage-2a table's frozen note, plus a row recording the one difference a maintainer would miss: the provisioner is **one** `migrate deploy` on the tag and **two** on head). Two stale forward-looking sentences swept. Final gate re-measured independently (2617 / 84 unit, 1387 + 2 e2e on jsonb, fixture 0 / control 1), and `prisma/` is unchanged since `c92b802`. **The remaining Phase 9 work is the operator's**: cutover steps 1–4, step 6, then stage 2b. |
| 2026-09-15 | v4.39 | ✅ **Phase 9 code work APPROVED** at `c92b802`; gate re-measured independently with `fondo_api_test` dropped and rebuilt (2617 / 84 unit, 1387 + 2 e2e on jsonb, fixture 0 / control 1). **C98 closed** — the reviewer *measured* that the subscription `findMany` still emits no `ORDER BY` and no `LIMIT`, so the heap order every SQS body depends on survived the move to Prisma models. **C99 closed.** **C100 opened as a standing instruction:** `migrate deploy` never verifies an applied migration's checksum — measured twice, including a full body replacement — so after step 6 an edit to those four files is invisible on production and executed everywhere built afterwards; freeze them, change by new migration. ⚠️ The reviewer's closing note had listed B4 and N10 as open; both were already closed (tag `release-a-cutover` + plan v4.38), and it confirmed that on re-measurement, naming the error as its own rule-15 failure. |
| 2026-09-15 | v4.38 | ✅ **C97 closed — Release A exists as an artifact.** `5d019d9` tagged `release-a-cutover`, annotated and frozen; cutover step 3 and the recovery path name it; I gated the tag independently (lint 0, `tsc` 0, 2619 / 82 unit, 1386 + 2 e2e on an `hstore` test database, fixture 0 / control 1). ⚠️ The first attempt was a **false red**: `fondo_api_test` still held head's converted schema, so Release A's `SchemaShapeGuard` refused to start and named both columns — Q57's guard doing its job. Recorded: gating either build must drop `fondo_api_test` first. **N10** folded in — the step-3a-bis sweep is marked pre-step-6 SQL. C98/C99 with `nestjs-developer`. |
| 2026-09-15 | v4.37 | 🟡 **Phase 9 stage 2a: `5d019d9` APPROVED, `204aa88` APPROVED WITH CONDITIONS; cutover NO-GO until C97.** The reviewer verified the highest-risk claims by measurement (empty `migrate diff` against the migrated clone, the five-statement diff against the un-migrated one, no bare `->` comparison anywhere, `hstore-legacy.ts` byte-identical to the deleted codec, C91's pin on the single production path). 🔴 **C97 — Release A is not an artifact:** cutover step 3 names no build, the design calls it "the same file with `'hstore'`", and the recovery row tells the operator to redeploy an image that does not exist. `5d019d9` is a coherent Release A but its gate was developer-reported only — the one build the fund would run for the entire cutover window is the one nobody re-measured. **C98/C99** — documentation that asserts the opposite of the code, and two seams to mark. |
| 2026-09-15 | v4.36 | 🟡 **Phase 9 stage 1 APPROVED WITH CONDITIONS** (`4db69fb`); **stage 2a is GO**. B1–B3 and M1–M7 closed; the reviewer re-measured the 17.11 migrated clone (3411 entries, 0 doubly-stringified, `owner_id` string ×626, 94/94 `keys` flipped, xmin 1/1, `0_init` still 0). **C94** — the e2e provisioner's second `migrate deploy` was labelled stage 2b in four places, which would leave stage 2a ungateable and push a developer into moving the step-6 directories (defeating C92). **C95** — preflight check 7 misses an expression index; measured, one on `akeys(payload)` aborts the conversion behind the masked error. **C96** — closing the leftover-env path with a dated confirmation variable instead of mitigating it. |
| 2026-09-15 | v4.35 | ✅ **C91 ruled (Q60): preserve `p256dh, auth`** — Release B pins the emit order, the conversion is unchanged. `business-analyst`'s evidence (`docs/ba-phase-9-step6-questions.md`): 94/94 stored in that order, no signature, content hash, FIFO dedupe or message attributes anywhere on the SQS path, web-push crypto uses the values not their order — but the consumer is an out-of-repo Lambda, so the operator took the no-change option at the door. **Q61:** no out-of-repo job reads `fondo_api_notificationsubscriptions` or `fondo_api_schedulertask` directly. **Q62:** the PWA re-subscribes only on explicit opt-in, so deleting a subscription row silently ends that member's push — runbook wording required. Confirmed: `owner_id` stays a string; the preflight's quote stop is right. |
| 2026-09-15 | v4.34 | 🔴 **Phase 9 stage 1: CHANGES REQUESTED.** The conversion SQL, the mutation controls and the C39 finding stand; the **runbook and release ordering** do not, and under Q56 they sit on the one-way door. **C92** — the step-6 directory stays outside `prisma/migrations` through Release B (a failed migration there would `P3009` every later deploy, Release A's included); **C93** — stage 2a lands before cutover step 3, because Q57 puts the mirror guard in Release A; **C91** 🔴 — the `keys` object reorders on the SQS wire for 94 of 94 subscriptions, measured, and must be decided while the stored order is still observable. Backup-before-quiesce, an unidentified target database, uncontrolled proof lines and dev-count verification are all in the fix round. |
| 2026-09-15 | v4.33 | 🟡 **Phase 9 stage 1 landed** (`de17e97`): measurements with no stop condition, four step-6 migrations, a row-by-row clone proof with a corruption control, mutation table (8 mutants aborted, 3 blind controls applied), and a release design. **Operator answers Q56–Q59:** deploys run migrations (so the step-6 directory stays separate until step 6 is done in production), Release A gets the mirror boot guard, the redundant Django indexes stay, and **production is PostgreSQL 17.x** — the proof must be re-run there. |
...
3. Deploy **the build tagged `release-a-cutover`** (commit `5d019d9`) with **`SCHEDULER_ENABLED` unset everywhere**; point DNS/proxy at it.
   ⚠️ **That tag is Release A — the last build that runs on the `hstore` schema v1 shares, and the one a rollback from Release B lands on.** Head is Release B and **must not** be deployed before step 6: its guard refuses to start on `hstore`. Release A is **frozen**; a fix on it is made on the tag, and must not carry head's second `migrate deploy` in `test/test-database.ts`, which would convert the hstore test database and silently invalidate the build. Gate measured independently 2026-09-15: lint 0, `tsc` 0, **2619 / 82** unit, **1386 + 2 skipped; 23 + 1 of 24** e2e on an `hstore` `fondo_api_test`, fixture diff 0 / control 1. 🔴 **This step closes D38's accepted v1 exposure** (the unauthenticated account takeover) — it ends here, not at step 6, and needs nothing extra done.
...
3a-bis. ⚠️ **Delete task 2142 before 14 November 2026 — no drain predicate reaches it** (Q51).
   ...
   ⚠️ **Pre-step-6 SQL.** After step 6 the column is `jsonb`, where `->` yields `jsonb` and both comparisons error — use `->>` then (review N10).
   ```sql
...
| C91 | **Step 6 reorders the `keys` object inside every push subscription, on the SQS wire.** `keys` is an opaque hstore *string* before and a jsonb *object* after; jsonb sorts keys by length then bytes, so all **94 of 94** rows go from `{p256dh, auth}` to `{auth, p256dh}` (measured by `nestjs-reviewer` on `fondodev` 2026-09-15). The project holds SQS bodies to a byte-identical criterion (§4 rules 8, 5d; Phase 2 round-trip cell). The proof pins **top-level** key order only — this is one level down. | P9 · review M1 | ✅ **Ruled by the operator 2026-09-15 (Q60) — preserve `p256dh, auth`.** Release B pins the emit order explicitly, with a cell; the conversion itself is unchanged. `business-analyst` measured: 94/94 rows in that order (the browser's own `PushSubscription.toJSON()` order), 0/94 containing a `"`, values plain base64url; no signature, hash, FIFO dedupe or `MessageAttributes` anywhere on the SQS path in either repo; RFC 8291 takes `p256dh`/`auth` as values and RFC 8292 signs its own JWT. The consumer is an out-of-repo Lambda, so the evidence could not settle it — the operator chose the no-change option at the one-way door. ⚠️ Had it been wrong, the failure is **silent**: the dead-subscription feedback loop was deleted in v1's `27e8551` (#91), so members simply stop receiving birthday greetings and T-5d/T-1d payment reminders. |
| C92 | **Release ordering under Q56 (deploys run migrations).** A failed step-6 migration inside `prisma/migrations` writes a failed ledger row, and **every later `migrate deploy` fails `P3009` — including a redeploy of Release A**, so the pipeline is stuck in both directions during an outage until `migrate resolve` is run by hand on production. The design's §6.1 table and its own Q-P9-1 note contradict each other on where the directories live. | P9 · review B2 | ✅ **Closed at `4db69fb`** — the design says one thing in §3.1, §6.1 and §9, with the P3009-in-both-directions consequence written in. ⚠️ Re-check found one residual (**C94**): the e2e provisioner change was labelled stage 2b, which would make stage 2a ungateable. |
| C93 | **Stage 2a must land before cutover step 3.** Q57 puts the mirror boot guard in **Release A**, which is what step 3 deploys; the design filed `SchemaShapeGuard` under stage 2, i.e. after step 6. | P9 · review M7 | 🟡 **Sequencing fixed:** stage 2a (codec deletion, Prisma models, `Json @db.JsonB` + the two `@unique`s, the guard in both directions, fail-closed cells) lands and is gated **before** cutover. |
| C94 | **The second `migrate deploy` in `test/test-database.ts` was labelled stage 2b in four places**, while its own trigger says "when Release B's code lands" — which is stage 2a. A developer starting stage 2a would find every e2e suite failing at bootstrap (`fondo_api_test` still hstore) and the shortest visible way out is `git mv` of the step-6 directories into `prisma/migrations` — C92 defeated by the document meant to enforce it. | P9 · re-check N1 | ✅ **Closed at `5d019d9`** — all four sites split correctly, and stage 2a made the provisioner change in the same commit as the codec deletion, so label and code agree (verified by `nestjs-reviewer`). |
| C95 | **Preflight check 7 cannot see an expression or predicate index** (`pg_attribute` joined on `a.attnum = ANY(i.indkey)`, and `indkey` holds `0` for an expression column). Measured by `nestjs-reviewer` on a scratch 17 clone: an index on `akeys(payload)` is invisible to the check and then **aborts the conversion** with `ERROR: current transaction is aborted` — the masked message the two-file split exists to prevent. `fondodev` has no such index and Django would not create one; production is unmeasured. | P9 · re-check N2 | ✅ **Closed at `5d019d9`** — the `pg_depend` sweep catches four kinds where the old check caught one (GIN, expression, predicate, CHECK, view). ⚠️ **Cross-version find:** PostgreSQL 18 materialises `NOT NULL` as a `pg_constraint` row and 17 does not, so `contype = 'n'` is excluded — re-measured by the reviewer on 18.6 and 17.11; without it the preflight would refuse to run on 18 against a healthy database. A cell pins that the exclusion was not widened to "ignore constraints". |
| C96 | **`PRISMA_MIGRATIONS_PATH` left in a task definition would convert production on a routine deploy** — and because Release A carries the step-6 directory through cutover by design, the deployed build's own guard would then refuse to start: the one-way door fires, the service is down and rollback to v1 is dead, from an unwatched deploy. Mitigated today by three runbook prohibitions, the resolved path printed on every invocation, and the guard's loud failure. `nestjs-reviewer` accepted the warn-on-non-default deviation as correct (warning on *set* would fire every e2e run). | P9 · re-check N5 | ✅ **Closed at `5d019d9`** — a non-default `PRISMA_MIGRATIONS_PATH` now requires `STEP6_CONFIRM` = today's date in Bogotá or the CLI exits 1 (pure function, 18 cells, measured through the real CLI in four states). A leftover goes stale at the next Bogotá midnight instead of converting production weeks later. |
| C97 | 🔴 **Release A is not an artifact.** Cutover step 3 says "Deploy v2" and names no build; the design says "the same file with `'hstore'`" (a source edit, not an image) and §6.7 tells the operator mid-incident to "redeploy the Release A image", which does not exist. Head is Release B. `nestjs-reviewer` confirmed `5d019d9` is a coherent Release A (guard `'hstore'`, codec present, baseline-ledger provisioner) — but its gate was **developer-reported only**: the one build the fund would run for the whole cutover window is the one nobody re-measured. | P9 · re-check B4 | ✅ **Closed 2026-09-15.** `5d019d9` tagged **`release-a-cutover`** (annotated: what it is, that it is frozen, and that a fix on it must not carry head's provisioner). **Gated independently by me on the tag**, not from the developer's report: lint 0, `tsc` 0, **2619 / 82** unit, **1386 + 2 skipped; 23 + 1 of 24** e2e, fixture diff 0 / control 1; HEAD unchanged and the tree clean, and the checkout returned to the branch with the client regenerated both ways. Cutover step 3 now names the tag, as does the recovery path. ⚠️ **First attempt was a false red worth keeping:** e2e failed 1306 of 1388 because `fondo_api_test` still held the **converted** schema from head's gate, and Release A's `SchemaShapeGuard` refused to start, naming both columns. The guard did exactly its job (Q57); the two builds cannot share a test database without rebuilding it, so a gate of either build must drop `fondo_api_test` first. |
| C98 | **Stale documentation that asserts the opposite of the code.** (N6) `docs/phase-9-design.md:804-805` says the e2e run is on the hstore schema "which is what Release A deploys", two lines under the correct statement that it runs on the converted schema and 24 cells changed meaning. (N7) `notification-subscription.repository.ts:14-50` still describes the deleted codec and links two deleted symbols — dangerous because the same block carries the **still load-bearing** *"row order is part of the SQS wire format — do not add an ORDER BY"*, which a maintainer may discount along with the rest. (N8) three more `{@link}`s to deleted exports (`prisma.service.ts:19-20,32`, `scheduler-task.repository.ts:152`, `python-json-dumps.ts:34`) — the one kind of rot `tsc` and lint cannot catch. | P9 · re-check N6–N8 | ✅ **Closed at `c92b802`** — the contradicting paragraph deleted, the subscription repository's block rewritten with the **ORDER BY paragraph first and verbatim**, and the `{@link}` sweep verified by the reviewer (a grep over the ten deleted codec exports returns nothing). ⚠️ The reviewer **measured** the ORDER BY property at head rather than reading it: the shipped client emits `SELECT … WHERE user_id IN (…) OFFSET $4` with **no `ORDER BY` and no `LIMIT`** on the `findMany` — Django's shape — so the heap order every SQS body depends on survived the move to Prisma models. |
| C99 | **Two runbook/code seams to mark.** (N9) the preflight deliberately accepts a **scalar** `user_ids` while `scheduler-payload.ts:74-79` throws on it — coherent (both stacks abort the publish and retry the row; 0 of 626 rows) but they read as contradictory, so each side must name the other. (N10) the step-3a/3a-bis departed-member sweep SQL is **pre-step-6 only**: after the conversion `->` yields jsonb and both comparisons error; §6.7 sends operators back into the runbook mid-incident. | P9 · re-check N9, N10 | ✅ **Closed at `c92b802`** — both sides of the scalar `user_ids` seam name the other; the plan's step-3a-bis sweep carries the pre-step-6 marker (N10). |
| C100 | **After step 6, an edit to any of the four step-6 migration files diverges two populations silently.** `migrate deploy` keys on the migration **name** and never verifies an applied migration's checksum. Measured twice — the developer appended a comment, the reviewer replaced an entire body: both give `No pending migrations to apply.` exit 0, `migrate status` says up to date, and the ledger `checksum` is untouched. But the same edited file **runs** on a database that has never seen it (`P3018` / `P0001`, verbatim, exit 1). So production keeps what it actually ran while CI, `fondo_api_test`, clones and any rebuild get the new text, with nothing comparing them. `migrate dev` is the only path that detects divergence and is **forbidden** here, so it is not a mitigation. | P9 · M-D34-8, re-check | ✅ **Recorded at `0bf23da`** as M-D34-8a / M-D34-8b, in §2's table **and** beside the runbook's step-6 block, with both measurements inline (the developer re-measured independently and got the same checksum `3582d8db7f5b930b`). `migrate dev` is named as the only command that would detect the divergence and explicitly not offered as a mitigation — *"the freeze is the mitigation, not a tool"*. Also recorded there: the same fact is why editing that file **today** is safe, since no production database has applied anything yet. 🟡 **Standing instruction for the operator from the moment step 6 commits.** |
...
### ▶ Now — Phase 9 complete in code at `0bf23da`; the cutover is the operator's to run.

| | |
|---|---|
| **Phase 9 — code work** | ✅ **COMPLETE AND APPROVED at `0bf23da`.** Every condition from both review rounds is closed. Final gate, measured by me with `fondo_api_test` dropped and rebuilt: lint 0, `tsc` 0, **2617 / 84** unit, **1387 + 2 skipped; 23 + 1 of 24** e2e on the **jsonb** shape, fixture diff 0 / control 1; HEAD unchanged, tree clean, and `git diff c92b802 0bf23da -- prisma/` is **empty**, so the four migration files are byte-identical to the reviewed ones. |
| **Release A — the build cutover deploys** | **`release-a-cutover` = `5d019d9`**, annotated and frozen. Gated by me on an **hstore** `fondo_api_test`: lint 0, `tsc` 0, **2619 / 82** unit, **1386 + 2 skipped** e2e, fixture 0 / control 1. ⚠️ Gating either build requires `dropdb fondo_api_test` first — the two builds cannot share one, and a count alone does not say which shape a run used. |
| **Operator sequence** | **3.** deploy `release-a-cutover`, `SCHEDULER_ENABLED` unset; check production's `TIME_ZONE` is unset or exactly `America/Bogota`. **3a / 3a-bis** drain, then **delete** tasks 1497 and 2142 (Q51) — pre-step-6 SQL. **3b** enable the scheduler on exactly one process. **4** smoke test. **6** quiesce → pre-image → backup → prove the restore → identify the database aloud → `PRISMA_MIGRATIONS_PATH=prisma/migrations-step6 STEP6_CONFIRM=$(TZ=America/Bogota date +%F) npx prisma migrate deploy` → `step6-verify.sh verify` must exit 0 → deploy Release B (head). Release B's refusal to boot on `hstore` is the check that step 6 ran. |
| **Open operator actions** | **C84** — compare `fonmon` against the live `File` rows before cutover. **EXEMPTED** — raise with the PRESIDENT before the 2026 activity marking (Q55). **C100** — from the moment step 6 commits, the four step-6 migration files are frozen. **Stage 2b** — only after step 6: move the directories in, delete the knob and `hstore-legacy.ts`. |
...
| 9 Cutover, hstore→jsonb | ✅ **CODE WORK COMPLETE AND APPROVED** (`0bf23da`) — every documentation condition closed; gate re-measured by me on a rebuilt `fondo_api_test`. **The cutover is the operator's to run**; stage 2b follows step 6 | ✅ | ⬜ n/a | ✅ **Approved** | ✅ two BA notes | C91–C99 ✅; **C100** standing instruction; **C84**, `TIME_ZONE`, EXEMPTED = operator actions |
...
| **Q56** | **Does production's out-of-repo `entrypoint_deploy` run `prisma migrate deploy` on a new build?** It decides whether the step-6 migrations may live in `prisma/migrations`. | ✅ **Answered 2026-09-15 — yes, deploys run migrations.** 🔴 So the step-6 directory **must stay out of `prisma/migrations` until step 6 has run in production**, or a routine deploy walks through the one-way door unattended. At cutover (step 3) `prisma/migrations` holds only `0_init`, which Prisma treats as applied — a no-op. The Release B sequence must be designed around an auto-migrating deploy. | P9 · Q-P9-1; D34/C39 |
| **Q57** | **Should Release A (the cutover build) also refuse to boot on an already-converted schema**, mirroring Release B's guard? | ✅ **Answered 2026-09-15 — yes, add the mirror check.** Without it a wrong-order rollback starts cleanly and fails only per request, on notifications and the unattended scheduler. | P9 · Q-P9-2 |
| **Q58** | **Drop the three Django indexes the conversion makes redundant, as part of step 6?** | ✅ **Answered 2026-09-15 — leave them.** They cost write overhead and disk only; dropping them adds change to the irreversible step for no functional gain. Backlog. | P9 · Q-P9-3 |
| **Q59** | **Which PostgreSQL major version does production run?** The migration proof ran on 18.6 locally; the repo records no production version. | ✅ **Answered 2026-09-15 — 17.x.** The step-6 migrations must be re-proved on PostgreSQL 17 before the runbook is final. | P9 · Q-P9-4 |
| **Q60** | **C91 — step 6 reorders the `keys` object on the SQS wire** for 94 of 94 subscriptions (`{p256dh, auth}` → `{auth, p256dh}`; jsonb sorts by length then bytes). The consumer is an out-of-repo Lambda. | ✅ **Answered 2026-09-15 — preserve `p256dh, auth`**, pinned in Release B by a cell. The no-change option at the one-way door. | **C91**; §4 rules 5d, 8 |
| **Q61** | **Does the Lambda, or any out-of-repo job, read or write `fondo_api_notificationsubscriptions` or `fondo_api_schedulertask` directly?** Any direct reader breaks at step 6. | ✅ **Answered 2026-09-15 — no; the Lambda only consumes the queue.** So step 6's blast radius is v1 and v2 alone, and the runbook needs no extra step for an external reader. | P9 · `business-analyst` |
| **Q62** | **Does the PWA re-subscribe on every load, or only on explicit opt-in?** It decides the cost of deleting a malformed subscription row — the analyst's suggested resolution for the preflight's `keys`-with-a-quote stop. | ✅ **Answered 2026-09-15 — only on explicit opt-in.** ⚠️ So deleting a subscription row **silently** ends that member's push until they turn notifications on again; the runbook must say to tell the member, and prefer stopping and consulting over deleting. 0 of 94 live rows need it today. | P9 · preflight stop (Q-P9-6) |
