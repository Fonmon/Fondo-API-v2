...
| **P8-D4** | uncaught 500 pages: gunicorn's 141-byte `text/html` (init-time multipart failures), Django's 27-byte `text/html` (`?type=abc`, storage errors on detail) | zero-byte 500, same absence of `Allow` and of `Accept` in `Vary`. ⚠️ **Not the same header set** on the init-time multipart 500s (empty, quoted trailing-space and non-ASCII boundary): v1's gunicorn page has **no `Vary`**, v2 sends **`Vary: Origin`** (measured, `docs/parity-phase-8.md` §1.4, §2.4). ⚠️ v1's page also has **no `X-Frame-Options`**, and v2 sends **`X-Frame-Options: SAMEORIGIN`**. `manual-tester` measured this on the empty and non-ASCII boundaries (`~/.fondo-parity-harness/p8r/out/c79-v2-8948f79.jsonl`, cases `E-mp-empty-boundary` and `E-mp-nonascii-boundary`). The quoted trailing-space boundary's `X-Frame-Options` was not measured. v2's header comes from the global hook in `src/common/http/django-response-headers.middleware.ts` | the existing **P3-D6** class; status parity kept. Both header differences are registered here with no fix: `Vary: Origin` is plan §7 **C85**, and `X-Frame-Options` is **C86** (ruled by `nestjs-reviewer` 2026-09-14). They are the same kind of header-set difference that **D24** registered on the same gunicorn double-fault page in Phase 3. Pinned by `expectUncaught500` (`vary === 'Origin'` and `x-frame-options === 'SAMEORIGIN'`) in the e2e boundary table |
