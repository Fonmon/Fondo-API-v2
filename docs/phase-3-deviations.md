...
| **P3-D6** | An **uncaught** exception renders Django's HTML 500 page: `<h1>Server Error (500)</h1>`, `Content-Type: text/html`, `Content-Length: 27`. | A zero-byte 500 with no `Content-Type` (the DRF-empty-body shape). | The same species as **D13** (v2 answers JSON/empty where Django answers HTML), extended to the 500 page. ⚠️ Note v1 has **two** different 500 bodies and v2 reproduces the other one exactly: `UserAppsView`'s `except Exception: return Response(status=500)` is a *DRF* response and really is zero bytes in both. |
