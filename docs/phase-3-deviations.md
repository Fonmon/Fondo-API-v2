...
| **R2.11.5** | A missing e2e **cell**, not a defect: `POST /password_reset/` with a non-empty *wrong* `csrfmiddlewaretoken` and a valid `X-CSRFToken` must be **403**, because Django's fallback fires on the empty string alone. Without it, an implementation that read the header first would satisfy every other F5 cell. Added with two positive controls. | `test/password-reset.e2e-spec.ts` |
