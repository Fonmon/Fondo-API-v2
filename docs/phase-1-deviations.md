...
| **P1-D2** | `OPTIONS /api-token-auth` returns DRF's browsable-API metadata document: `200 {"name":"Obtain Auth Token","description":"","renders":[…],"parses":[…]}`. | `405 {"detail":"Method \"OPTIONS\" not allowed."}`. | v2 ships no browsable API and no `OPTIONS` metadata layer. Nothing in the fund's front end issues a bare `OPTIONS`; CORS preflights are handled by the CORS middleware before routing. Revisit only if a client turns out to depend on it. |
