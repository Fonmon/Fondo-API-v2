 ...
 *
 * The ranges are ascending, non-overlapping (enforced by `python-str.fixture.spec.ts`) and flat (`[start, end, start, end, …]`), so the
 ...
 * `scripts/gen-python-str-fixture.py`, ascending, non-overlapping (enforced by `python-str.fixture.spec.ts`), flat `[start, end, …]`
 ...
 * i.e. any decimal digit, not `[0-9]`. Built from the same pinned table the fold uses.
 *
 * ⚠️ Sharing the table does not by itself keep the two in agreement (Major 9, measured): this
 * class is order-blind — a character class is a set — while the fold binary-searches and needs
 * the table ascending, so a mis-ordered table widens the class alone. Agreement on every code
 * point, and the table's order, are enforced by `python-str.fixture.spec.ts`.
 */
export const PYTHON_DIGIT_CLASS = ((): string => {
  let out = '';
...
export function parseStrptimeIsoDate(
  ...
  // Defensive, and not the UCD-drift countermeasure (that is the Major 5 matrix). Class/fold
  // agreement is enforced by `python-str.fixture.spec.ts`; this guard keeps the contract —
  // never a NaN part — from depending on that test staying green.
