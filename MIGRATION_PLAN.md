...
| C82 | **P8-F1–F3 — keep v1's partial writes or change them?** Cross-type duplicate name leaves an orphaned object; a name differing only in case silently replaces another row's object; `type` is unvalidated. | P8 · findings | 🟡 **With `business-analyst`**, then operator |
...
### ▶ Now — Phase 8 (Files + admin) IMPLEMENTED; awaiting `manual-tester`.

| | |
|---|---|
| **Phase 8 — Files + admin** | 🟡 **Implemented at `71cd7de`** on `feat/phase-8-files`: `GET\|POST /api/file`, `GET /api/file/<id>`, `GET /api/admin`; v1's 8 file tests ported; admin covered from measurement (v1 has no admin tests). Next: `manual-tester`, then `nestjs-reviewer`. |
| **Gate — re-measured by me, each step's own exit code** | lint 0 · `tsc` 0 · **2482 unit / 77** (was 2331 / 71) · **1297 e2e passed + 2 skipped; 22 suites passed + 1 skipped of 23** (was 1177 + 2; 20 + 1 of 21) · fixture diff 0 / control 1. The developer reports 38 of 38 mutation controls killed. |
| ⚠️ **The fixture gate was blind to this phase's table** | `fixture-check.sh` had no count, max-id or `xmin` row for `fondo_api_file` — only its sequence. Measured directly: **37 rows, max id 43, `xmin` cardinality 1, sequence 43**, matching the developer and the saved baseline. Extended at `4ab180a`; the new output differs from the old baseline by exactly those three rows, and the new baseline (`~/.fondo-parity-harness/p8/out/BASELINE-fondodev-2026-09-12.txt`) diffs 0 with a control of 1. |
| **Decisions open from this phase** | **C79** — D22 on `POST /api/file`, where its rationale does not hold. **C80 / D45** — U+2028/U+2029 escaping in all JSON. **C81** — measure P8-F5 on the approved Phase 3 and 4 bulk uploads. **C82** — P8-F1–F3 partial writes, with `business-analyst`. |
| ⚠️ **Decision needed — C71 and D39 have no owning phase** | Both are birthday-notification work left out of Phase 6, and neither is files or admin. **C71**: a birthdate set after the member's birthday creates an already-past-due task v2 would fire on the wrong day. **D39**: v2 must not announce a departed member's birthday (Q35); the filtering layer is undecided. |
...
| 8 Files + admin | 🟡 **IMPLEMENTED** (`71cd7de`) — gate re-measured independently; awaiting `manual-tester` | ✅ | ⬜ | ⬜ | 🟡 P8-F1–F3 with analyst | C79–C83 open (D22 on this route, D45, P8-F5 on Phase 3/4 routes, partial writes, path-id hoist); fixture check extended `4ab180a` |
