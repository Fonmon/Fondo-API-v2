...
**§5 rows this phase owns:** **D39**. **Conditions:** **C71**.
...
- **D39** — v2 must not announce a departed member's birthday (Q35). The filtering layer is
  undecided: filtering in the executer still marks the row processed and clones the successor, so a
  returning member resumes automatically; filtering at chain creation does not. Decide and record
  it in D39.
...
| C79 | **Does D22 stand on `POST /api/file`?** There v1 answers every invalid boundary with 500, so D22's rationale does not hold, and applying it yields v1 500 / v2 400 for a non-ASCII boundary only. | P8 · raised by `nestjs-developer` | 🟡 **Open** — `nestjs-reviewer` ruling at Phase 8 review; operator if it becomes a business call |
| C80 | **D45: escape U+2028/U+2029 in JSON like DRF, or register the difference permanently?** Affects any response carrying user text with those characters, in every phase. | P8 · P8-F4 | 🟡 **Open** — `nestjs-reviewer` ruling; a fix touches approved phases |
| C81 | **Measure P8-F5 on the approved routes.** `readUploadedFile` now takes the last same-named part; that was measured on `POST /api/file` only. Measure `PATCH /api/user` (Phase 3) and `PATCH /api/loan` (Phase 4) with two `file` parts against v1. | P8 · P8-F5 | 🟡 **Open → Phase 8 parity round** |
| C82 | **P8-F1–F3 — keep v1's partial writes or change them?** Cross-type duplicate name leaves an orphaned object; a name differing only in case silently replaces another row's object; `type` is unvalidated. | P8 · findings | 🟡 **Decided by the operator 2026-09-12** — F1 refuse before upload (**D46**, Q40); F2 keep v1 (Q41); F3 refuse unknown types (**D47**, Q42). D46/D47 implementation pending in Phase 8. |
| C83 | Three copies of the out-of-range path-id rule (`loans/loan-path-id.ts`, `activities/activity-path-id.ts`, `common/http/django-int-path-id.ts`, counted at `71cd7de`); hoist the first two onto the common one. | P8 · fold-back 7.7 | ⬜ Non-gating follow-up |
...
### ▶ Now — Phase 8: operator decisions taken; D46/D47 to implement, then the parity round.

| | |
|---|---|
| **Phase 8 — Files + admin** | 🟡 **Implemented at `71cd7de`**; gate re-measured independently. **Operator decisions taken 2026-09-12:** refuse a wrong-type duplicate before upload (**D46**, Q40); keep v1's silent same-type replacement (Q41); refuse unknown types (**D47**, Q42). **Next:** `nestjs-developer` implements D46/D47, then `manual-tester` runs the parity round once on the final code, then `nestjs-reviewer`. |
| **Parity round interrupted** | `manual-tester` hit the session limit before producing anything. Checked afterwards: the v2 repo was clean at `210c72f`, no container was running, and nothing new was written under `~/.fondo-parity-harness/` or `~/.fondo-parity-dumps/`. |
| **Decisions open from this phase** | **C79** — D22 on `POST /api/file`. **C80 / D45** — U+2028/U+2029 escaping in all JSON. **C81** — measure P8-F5 on the approved Phase 3 and 4 bulk uploads. **C83** — path-id hoist (non-gating). |
| **Phase 8b — Birthday notifications** | ⬜ **Queued** after Phase 8, before Phase 9 (Q43): **C71** and **D39**. |
| ⏰ **Dated deadline** | **Task 2142 fires 14 November 2026** — Angi Paola, `is_active = f`; under **Q35** it must not fire, and no drain predicate reaches it. It fires **under v1 too** if cutover has not happened by then. Runbook **step 3a-bis**. |
...
| 8 Files + admin | 🟡 **IMPLEMENTED** (`71cd7de`); operator decisions Q40–Q42 taken, **D46/D47 to implement** before the parity round | ✅ (D46/D47 pending) | ⬜ round interrupted by a session limit; to re-run on the final code | ⬜ | ✅ `ba-phase-8-files.md` | C79–C81, C83 open; C82 decided (D46, Q41, D47) |
| 8b Birthday notifications | ⬜ **Queued** — after Phase 8, before Phase 9 (Q43) | — | — | — | — | **C71**, **D39**; task 2142 deadline 14 November 2026 |
