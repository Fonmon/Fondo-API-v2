...
| C92 | **Release ordering under Q56 (deploys run migrations).** A failed step-6 migration inside `prisma/migrations` writes a failed ledger row, and **every later `migrate deploy` fails `P3009` — including a redeploy of Release A**, so the pipeline is stuck in both directions during an outage until `migrate resolve` is run by hand on production. The design's §6.1 table and its own Q-P9-1 note contradict each other on where the directories live. | P9 · review B2 | 🟡 **Decided by the review, to be implemented:** `prisma/migrations-step6` stays **outside** `prisma/migrations` through Release B; step 6 is an operator-run `migrate deploy` against that directory, never a deploy; the directories move in only afterwards (stage 2b). `test/test-database.ts` provisions `fondo_api_test` with two sequential `migrate deploy` runs rather than moving the directory. |
...
| **`nestjs-reviewer` (Phase 9 stage 1)** | 🔴 **CHANGES REQUESTED.** Re-measured the headline numbers on `fondodev` (626, 94, 626/626 arrays, 0/94 quotes, 0 duplicate groups, exactly two hstore columns, no dependent index or view) and called the mutation suite better than the Phase 6/7b standard. **Blockers:** **B1** the runbook takes the backup *before* quiescing, so a write in that window is in neither the backup nor any rollback; **B2** under Q56 a failed step-6 migration inside `prisma/migrations` bricks every later deploy, Release A included (**C92**); **B3** §6.2 never identifies the database it converts irreversibly — this repo's own `.env` points at `fondodev`. **Majors:** **M1** the `keys` object reorders on the SQS wire for 94/94 subscriptions (**C91**); M2/M3 preflight gaps (a doubly-stringified `user_ids`, dependent objects) that stop with the *masked* error; M4 verification hardcodes dev counts and cannot fail; M5 recovery text points at success-state SQL and skips the duplicate re-check on a retry; M6 the proof's `reverse` measures xmin cardinality (always 1 after a restore) and three of four PASS lines have no control; M7 the guard must require exactly two rows and land in Release A before cutover (**C93**). |
...
| 9 Cutover, hstore→jsonb | 🔴 **STAGE 1 CHANGES REQUESTED** (review of `de17e97`) — migration SQL, mutation controls and the C39 finding approvable; the runbook and release ordering are not. Fix round running | ✅ stage 1 | ⬜ | 🔴 CR | ✅ `ba-phase-9-c49-redecisions.md` | **C91** 🔴 decide before step 6; **C92**, **C93** sequencing; C39 answerable; C83, C90 cleanup; C84 + `TIME_ZONE` + EXEMPTED operator actions |
