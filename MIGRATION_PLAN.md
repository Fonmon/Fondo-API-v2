...
### ▶ Now — Phase 6: fourth fix round landed; awaiting `nestjs-reviewer` re-check #4.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 `cda69e0`; fixes `6a859d0`, `aa2fa56`, `886cbf5`, `b035db3`, round 4 on top |
| **Pipeline** | ✅ dev → ✅ tester PASS w/ 3 → 🔴 CR → ✅ → 🔴 CR → ✅ → 🔴 CR (M4, M5) → ✅ → 🔴 **CR (M6, M7)** → ✅ fix 4 → ⬜ **re-check #4** |
| **Gate** | lint ✅ · `tsc` ✅ · **2328 unit / 70** (was 2319) · **1175 e2e + 2 skipped / 21** · fixture ✅. Every one by exit code; **lint failed twice more this round and both were caught.** |
| 🔴 **Major 6 — a live parity defect on a write path** | `power.service.ts` was a **THIRD** hand-rolled port of Django’s date coercion, and the only date site in `src/` not routed through `toDjangoDate`. **Six divergences**; the last three **wrote a row v1 refuses and sent the notification that follows it** — `2020-02-30` → **2020-03-01**, `2020-13-01` → **2021-01-01**, `0000-01-01` → **year 0**. ⚠️ Missed by B1, B2 **and** Major 5 because it was never spelled `strptime` and never spelled `Date.UTC`; my Major 5 write-up said *the two hand-rolled ports are gone* when there were three. ⚠️ **The one negative cell that existed could not see any of it.** Registered **D44**. Control: old body → **exactly the 6** new cells fail. |
| 🟠 **Major 7 — an axis named, its boundary unpinned** | `%d`’s Unicode-awareness is `[1-2]\d`’s **alone**; `0[1-9]` and `3[0-1]` are ASCII in both characters. A counter-mutant taking the natural over-reading passed **all 2319** tests while turning a `0`-prefixed Unicode day into a 200-with-a-row where v1 raises — and a spec docblock **argued for exactly that wrong side** while four other sites said the opposite. Two refusing rows added; the counter-mutant now fails. |
| ⚠️ **A control I added does not control what I claimed** | Minor 8 asked for a UCD-drift guard on the `strptime` path. I added the cell, then tested it: under a Unicode-property class — **and with the `NaN` guard removed as well** — it still passes. Two independent mechanisms close that hole, so **on this path the digit-class choice is not behaviourally observable at all**. Relabelled a **regression pin, not a control**, pointing at the `toDjangoDate` path where the drift *is* observable. Shipping a row that looks like a control and is not is the failure this phase keeps producing. |
| **Also closed** | **Minor 9** — `setYear`/`getYear` is a **fifth** member of the class (`new Date(0).setYear(50)` → 1950), plus `globalThis.Date` and assignment aliasing: **8 of 8** bypass forms caught, 0 false positives. **Minor 10** — `date.util.ts` prose widened from one spelling to the class. **Nits 1–3** — `slice` not `trim()`, the anchor assumption stated as format-specific, and the OID sidecar’s reach stated honestly rather than overclaimed. |
