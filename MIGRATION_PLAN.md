...
| C79 | **Does D22 stand on `POST /api/file`?** There v1 answers every invalid boundary with 500, so D22's rationale does not hold, and applying it yields v1 500 / v2 400 for a non-ASCII boundary only. | P8 · raised by `nestjs-developer` | 🟡 **Ruled 2026-09-14 — exclude `POST /api/file` from D22**; v2 answers 500 like v1. Implementation in Phase 8's fix round (review M1), with the mutation set re-run on the changed files. |
...
| C85 | **`Vary: Origin` on init-time multipart 500s.** On `POST /api/file` with an unparseable multipart body (an empty boundary, or a quoted trailing-space boundary), v1 serves gunicorn's page with **no `Vary` header** and v2 sends **`Vary: Origin`**. Status matches; `docs/phase-8-deviations.md` had claimed the headers matched too. Measured by `manual-tester`. | P8 · parity round | 🟡 **Ruled 2026-09-14 — belongs to P8-D4, no fix** (v1's missing `Vary` is a side effect of the double fault escaping Django before CORS, as D24 already registered for that gunicorn page). Still required in Phase 8's fix round: pin `Vary: Origin` in the test and name the difference in P8-D4 (review m1). |
...
### ▶ Now — Phase 8 review: changes requested (narrow); fix round next.

| | |
|---|---|
| **Phase 8 — review verdict** | 🔴 **CHANGES REQUESTED**, narrow. `nestjs-reviewer` re-measured the gate on `ba5d6db` (lint 0, `tsc` 0, 2517 / 77 unit, 1331 + 2 e2e, fixture diff 0 / control 1), confirmed no code changed after `5f58b11`, matched the §5.1 mutation hashes, and regenerated the `pythonLower` fixture to an identical file. |
| **Rulings** | **C79** — D22 does not apply to `POST /api/file`; v2 answers 500 like v1. **C80 / D45** — registered permanently, no fix; parity compares parsed JSON when a body contains U+2028/U+2029. **C85** — belongs to P8-D4, no fix, but must be pinned by a test. **C83** — non-gating, Phase 9 cleanup. |
| **Required to approve** | **M1** remove the D22 branch and its helper, add the non-ASCII boundary to the uncaught-500 table, invert its mutant; **m1** assert `Vary: Origin` and correct the test title that says "without `Vary`"; **m2** rewrite a universal comment as the measured list. Then the full gate and the §5.1 mutation set re-run on the new file hashes. Rule-15 wording fixes (m4, m5, nits) and the 24 cosmetic `NaN` test titles ride along. |
| ⚠️ **Storage in development (m3)** | In any environment except tests, v2 uses the machine's Google credentials and defaults the bucket to production `fonmon`, as v1 does. This host has none (measured). Whether to refuse storage outside production is with the operator before the fix round. |
| **Phase 8b — Birthday notifications** | ⬜ **Queued** after Phase 8 is approved. Decided 2026-09-14: **D48** next birthday not yet missed (Q47), **D39** skip a departed member at send time while the chain keeps rolling (Q48), **D49** recipients resolved at send time (Q49). |
...
| 8 Files + admin | 🔴 **CHANGES REQUESTED** (review of `ba5d6db`) — fix round next: remove the D22 branch (M1), pin `Vary: Origin` (m1), rewrite one universal comment (m2); then gate + mutation re-run | ✅ | ✅ **PASS** | 🔴 CR (narrow) | ✅ `ba-phase-8-files.md` | C80, C81, C82 ✅; C79, C85 ruled, fix pending; C83 non-gating; C84 operator action |
