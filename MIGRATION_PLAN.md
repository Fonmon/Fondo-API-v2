...
### ▶ Now — Phase 6: round 6 landed; awaiting `nestjs-reviewer` re-check #6.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 `cda69e0`; review fixes `6a859d0` → `2811b56`; round 6 on top |
| **Pipeline** | ✅ dev → ✅ tester → 🔴 CR ×5 with a fix each → ✅ **round 6** → ⬜ **re-check #6, gated on exit codes, not prose** (the reviewer's terms) |
| **Gate** | lint 0 · `tsc` 0 · **2331 unit / 71 suites** (was 2328 / 70: +3 cells, +1 suite) · **1177 e2e + 2 skipped / 21 suites** · fixture ✅ **diff-clean against `FINAL2-fondodev.txt`** (the tester's end-of-Phase-6 capture, 2026-09-08), **controlled**: the same probe against `fondo_api_test` differs (`diff` exit 1). Every one by exit code. ⚠️ **The first gate run reported `E2E_EXIT=1` and `FIXTURE_EXIT=2` inside a background task whose own status read "exit code 0"** — `fondo_db` had exited three days earlier and e2e `globalSetup` died before a single cell ran. Read off the per-step exit codes, not the wrapper; database restarted, gate re-run. |
