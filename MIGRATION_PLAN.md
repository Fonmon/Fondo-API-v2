...
### ▶ Now — Phase 6: third fix round landed; awaiting `nestjs-reviewer` re-check #3.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 `cda69e0`, fixes `6a859d0` / `aa2fa56` / `886cbf5`, round 3 on top |
| **Pipeline** | ✅ dev → ✅ tester PASS w/ 3 → 🔴 CR → ✅ fix 1 → 🔴 CR → ✅ fix 2 → 🔴 **CR (Majors 4, 5)** → ✅ fix 3 → ⬜ **re-check #3** |
| **Gate** | lint ✅ · `tsc` ✅ · **2319 unit / 70** (was 2309) · **1175 e2e + 2 skipped / 21** (was 1174) · fixture ✅. Every one by exit code. |
| 🟠 **Major 5 — I pinned the WRONG rule with four cells and a register row** | I measured **one** string (`strptime('٢٠١٨-٠١-٠١')` raises) and generalised it to *“the `strptime` ports keep ASCII-only `\d`”*. `nestjs-reviewer` dumped the generated pattern: **`_strptime` is not one parser, it is a per-directive pattern.** `%Y` is `\d\d\d\d` — **Unicode-aware and folded**; `%m` is ASCII in every branch; `%d` is ASCII in its first character, **Unicode-aware in `[1-2]\d`'s second**, and carries a **space-padded ` [1-9]`** branch. The fully-Arabic string fails **because of the month**. **Seven** measured inputs were v1-accepts / v2-refuses, two on write paths. ⚠️ **A divergence a test defends is worse than one nobody noticed** — the cells argued for it. Closed by `parseStrptimeIsoDate`, mirroring the generated pattern; 11-row matrix, **controlled by two disjoint mutants** (ASCII-only class → **5** failures; drop the space branch → **2**; 5 + 2 = the 7 accepting cells). |
| 🟠 **Major 4 — my lint rule closed one spelling of four** | Planted five bypass forms; **the rule caught one**. `Date['UTC']`, destructuring, aliasing and — the real gap — **`new Date(y, m, d)`**, which runs the same `MakeFullYear` (measured: `new Date(50,0,1).getFullYear() === 1950`), is *more* idiomatic, and adds a local-timezone bug. ⚠️ **And my control proved the rule FIRES, not that the class is CLOSED** — C67 one level up: a control exercising only the case I had already thought of. Now **four selectors**, controlled against all five forms, **0** false positives on `new Date(millis)` / `new Date(iso)` / `new Date()`. |
| ⚠️ **My arithmetic was wrong again** | I said the `\p{Nd}` count moved 3 → 4 because of *both* Minor 5 and a new drift cell. Under the mutant `pythonInt` already throws, so the cell fails once — **Minor 5 contributed 0**. 3 + 1 = 4, the 1 being the `toDjangoDate` cell alone. Same species as the original: **a count attributed to a mechanism that did not produce it.** |
| **Minor 6 — the lesson generalised** | *“Assert the row, not the status”* was the tactic. The property is **a cell must be shown to fail against the mutation it claims to catch** — a row assertion is no safer if the row already holds the expected value from a fixture. Restated in the spec, and the one cell that asserted only a status now reads the row. |
| **Nit 3** | The `hstore` OID guard now pins to a **sidecar beside the dump**, so it spans the clone's whole life rather than one restore — the likelier 2am error is a `dropdb` by hand *before* the script runs. |
