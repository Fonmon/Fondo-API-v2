...
### ▶ Now — Phase 8 implemented (D46/D47 in); awaiting `manual-tester`.

| | |
|---|---|
| **Phase 8 — Files + admin** | 🟡 **Implemented**: `GET\|POST /api/file`, `GET /api/file/<id>`, `GET /api/admin` at `71cd7de`; operator decisions **D46** (409) and **D47** (400) at `5f58b11`. **Next:** `manual-tester` parity round on the final code, then `nestjs-reviewer`. |
| **Gate on `5f58b11` — re-measured by me, each step's own exit code** | lint 0 · `tsc` 0 · **2517 unit / 77** · **1331 e2e passed + 2 skipped; 22 suites passed + 1 skipped of 23** · fixture diff 0 / control 1. The committed D46/D47 source files match the hashes the mutation table was recorded against. |
| **Operator follow-ups (2026-09-14)** | **Q44** keep D46's refusal in the `Dup X` edge. **Q45** leave the concurrent-upload window as a known residual. **Q46** keep 400 for a `type` sent as a file part. |
| **Open from this phase** | **C79** — D22 on `POST /api/file`. **C80 / D45** — U+2028/U+2029 escaping in all JSON. **C81** — measure P8-F5 on the approved Phase 3 and 4 bulk uploads. **C83** — path-id hoist (non-gating). **C84** — before cutover, compare `fonmon` against the live file rows (operator action). |
...
| 8 Files + admin | 🟡 **IMPLEMENTED** — `71cd7de`, D46/D47 `5f58b11`; gate re-measured independently; awaiting `manual-tester` | ✅ | ⬜ | ⬜ | ✅ `ba-phase-8-files.md` | C82 ✅ closed; C79–C81, C83, C84 open |
