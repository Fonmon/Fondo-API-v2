...
### ▶ Now — Phase 8 (Files + admin) IN PROGRESS.

| | |
|---|---|
| **Phase 6** | ✅ **CLOSED — approved** (re-check #7, `5194a29`; closure `560df45`). |
| **Phase 8 — Files + admin** | 🟡 **In progress.** `GET\|POST /api/file` (POST ADMIN, GET MEMBER), `GET /api/file/<id>` (MEMBER), `GET /api/admin` (ADMIN). Branch `feat/phase-8-files` from `560df45`. Pipeline: dev → `manual-tester` → `nestjs-reviewer`. |
| **Measured before briefing (2026-09-12)** | Read from v1's source and `fondodev`, not paraphrased from §3. (1) `save_file` checks `blob.exists()` but **uploads unconditionally** (overwrites) and skips only the `File` row; the row is written after the upload, so a failed upload is 500 with no row. (2) `FileView.post`'s presence check reads the **merged** `request.data`, so a scalar field named `file` passes it and then fails on `.content_type` → **v1 500**; reading files only (rule 12c) would answer **400** — a parity trap of D23's shape. (3) `display_name` is unique across types while the object path includes the type, so uploading an existing name under the other type **stores the object and then 500s on the row**. (4) `GET /api/admin`: only a missing `type` is 400; any other value, empty included, is **200 and does nothing**; the self-test email and push go to **the caller only**. (5) `GET /api/file?type=` goes through `int()` **outside** the handler's try — an uncaught 500 on a bad value. (6) `fondodev`: **37 files** (23 type 0, 14 type 1), **one `created_at` tie of 2 rows**, so the list's `ORDER BY created_at` is not deterministic for them; 23 of 37 names are non-ASCII. (7) Python `str.lower()` (CPython 3.9.25, UCD 13.0.0) and Node `toLowerCase()` (Node 24.20.0, Unicode 17.0) **differ on 95 code points**; **0 of the 37 live names** are affected, so only a future upload could land at a different object path. (8) **No GCS credentials are reachable from this host** — no Application Default Credentials, no gcloud config, no gcloud CLI — and `@google-cloud/storage` is not yet a dependency. |
| ⚠️ **Decision needed — C71 and D39 have no owning phase** | Both are birthday-notification work left out of Phase 6, and neither is files or admin. **C71**: a birthdate set after the member's birthday creates an already-past-due task v2 would fire on the wrong day (Q36 folded Ainhoa's missing chain into it). **D39**: v2 must not announce a departed member's birthday (Q35); the filtering layer is undecided. |
...
| 8 Files + admin | 🟡 **IN PROGRESS** — `nestjs-developer` briefed on `feat/phase-8-files` (from `560df45`) | 🟡 | ⬜ | ⬜ | ⬜ | Inherits D22/D23 and rules 12b/12c; pre-brief measurements in ▶ Now |
