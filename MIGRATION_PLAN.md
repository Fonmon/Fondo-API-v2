...
| **Pipeline** | Phase 7b ✅ dev → ✅ **`manual-tester` PASS** (`3659eef`) → ✅ **`nestjs-reviewer` approved w/ conditions C68–C76** → 🟡 C74 closed, validation round running |
...
| **In flight** | 🟡 `business-analyst` on **C71** (D7's forward-going birthday face — a birthdate corrected after the fact pushes *"Hoy está cumpliendo años X"* to the whole roster on the wrong day, and **draining the backlog does not fix it**) and **C72**'s policy half (task 1497's disposition; Q34's wording, finding F4). 🟡 `manual-tester` + `nestjs-reviewer` validating C74. |
| **Next phase** | **Phase 6 — Saving accounts (CAPs).** Start is unblocked. It inherits the zero-instance failure mode wholesale: no flag means CAPs silently stop closing exactly as reminders silently stop sending, and **C68/C70 are the detection story for both**. |
