...
### ▶ Now — Phase 6 (Saving accounts / CAPs) **IMPLEMENTED**; awaiting `manual-tester`.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps`, head **`cda69e0`**, branched from `f7b7a46` |
| **Gate** | lint ✅ · `tsc` ✅ · **2274 unit / 70 suites** (was 2173 / 67) · **1160 e2e + 2 skipped / 21 suites** (was 1061 + 2 / 20) · `fondodev` `SELECT`-only and at baseline. **Every one asserted by exit code** (C67), and **re-measured independently by me**, not accepted from the report. |
| **Pipeline** | Phase 7b ✅ CLOSED → **Phase 6 ✅ implemented** → ⬜ `manual-tester` → ⬜ `nestjs-reviewer` |
| **Shipped** | `src/saving-accounts/` (controller, service, serializers, module) + 98 e2e cells; **D12** — `saving-account-close.executer.ts`, `createCloseSavingAccountTask`, `closeAccount`, `findUnclosedPastDue`, `ExecuterFactory` type 1. **C68, C69, C70, C73, C75, C76 closed**; C77/C78 code obligations discharged. |
| **D12 decided** | **One `SchedulerTask` per CAP, at creation, `repeat = 0`.** The sweep alternative was rejected on the developer's own generalisation of C67: ***a check that drives the work is not a check*** — a broken close would return an empty result and read as "nothing to do". Accepted consequence: a CAP created between passes with an `end_date` of today closes at **14:00**, no code. |
| ⚠️ **Two of my briefs were falsified, by mutation** | **C77(iii)** was vacuous as I worded it — a runner mutated to clone before running passes all 98 e2e cells; `createRepeatInstance` returns early on `NONE`, so "writes no clone" holds under either ordering. **C76**'s suggested cell did not discriminate — a `fromDateColumn → toBogotaDate` mutant passed it. Both fixed and now controlled. |
| **Open** | 🟡 **C71** and **D39** (birthday work, deliberately not this phase) · **P6-F2** — `PUT { state: 7 }` makes a row invisible to the list, to `total_savingaccounts` **and** to D12's close at once; ported, since `Model.save()` never validates `choices` |
| ⏰ **Dated deadline** | **Task 2142 fires 14 November 2026.** Angi Paola is `is_active = f`; under **Q35** it must not fire, and no drain predicate reaches it. Runbook **step 3a-bis**. ⚠️ **The one open CAP (id 3, 900,000) belongs to the same departed member** — closing it notifies nobody, but a reviewer should know whose money it is. |
...
| 6 Saving accounts (CAPs) | ✅ **IMPLEMENTED** (`cda69e0`, `feat/phase-6-caps`) — awaiting tester | ✅ | ⬜ | ⬜ | ✅ Q19–Q24, Q38, Q39 | **C68, C69, C70, C73, C75, C76 ✅ closed**; **C77, C78 code obligations ✅ discharged**; **D12** built, **D40** registered; six deviations **P6-F1**–**P6-F6**. **C71**, **D39** deliberately out of scope (birthday work) |
