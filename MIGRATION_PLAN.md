...
### ▶ Now — Phase 6: round 7 landed; awaiting `nestjs-reviewer` re-check #7.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 `cda69e0`; review fixes `6a859d0` → `2811b56`; round 6 on top |
| **Pipeline** | ✅ dev → ✅ tester → 🔴 CR ×5 with a fix each → ✅ round 6 → 🔴 **re-check #6: CR on one measurement** → ✅ **round 7** → ⬜ **re-check #7, exit codes only** |
| **Gate** | lint 0 · `tsc` 0 · **2331 unit / 71 suites** (was 2328 / 70: +3 cells, +1 suite) · **1177 e2e passed + 2 skipped; 20 suites passed + 1 skipped of 21** · fixture ✅ **diff-clean against `FINAL2-fondodev.txt`** (the tester's end-of-Phase-6 capture, 2026-09-08), **controlled**: the same probe against `fondo_api_test` differs (`diff` exit 1). Every one by exit code. ⚠️ **The first gate run reported `E2E_EXIT=1` and `FIXTURE_EXIT=2` inside a background task whose own status read "exit code 0"** — `fondo_db` had exited three days earlier and e2e `globalSetup` died before a single cell ran. Read off the per-step exit codes, not the wrapper; database restarted, gate re-run. **Round 7 changes only `eslint.config.mjs` (`git diff --stat`)**, so unit, e2e and the fixture diff carry from `3b20428`; lint and `tsc` re-measured, both 0. |
| 🔴 **Major 9 — an unchecked invariant, and my account of it was half wrong** | `PYTHON_DECIMAL_DIGIT_RANGES` has **two consumers with different preconditions**: the digit class is order-blind (a character class is a set), the fold binary-searches and needs the array ascending. So a range appended out of order **widens the class alone** — which is exactly the mutant I ran in round 5 and wrote off as *unreachable, so it tested nothing*. It was reachable to the class; it passed because nothing asserted the order. **Now a check:** `python-str.fixture.spec.ts` asserts the shape of both pinned tables and sweeps class-vs-fold agreement over all 1,114,112 code points (1.2 s). **Control:** the same out-of-order append fails 2 of its 3 cells — `pair 61 starts at 0x1e4f0, not after 0x1fbf9` and `U+1E4F0…U+1E4F9` — and restoring the fixture byte-identically returns 3 of 3. |
| 🔴 **Major 10 — escape sites by negation, not by spelling** | The reviewer planted nine forms past the previous selectors; eight escaped, including `new (Date satisfies DateConstructor)(1950, 0, 1)`, which drew no lint error of any kind. Two selectors replace one-spelling-at-a-time enumeration with a negation over every lexical `Date` outside an allowlist. **Measured on a per-line probe:** 10 of 10 plants caught (the nine plus `(Date as any)['U'+'TC']`), the stated survivor `(globalThis as any)['Da'+'te']` clean, 6 of 6 legitimate forms clean, repo lint 0. ⚠️ **The boundary sentence was refuted by its own example** — it named `(Date as any)['U'+'TC']` as the survivor, and the negation catches it. Corrected to the name never being spelled. ⚠️ **Re-check #6 refuted part of this:** the negation matched `Date` by parent node, not role — `new Proxy(Date, {})`, `new Box(Date)` and `Date.bind(null)` were lint-clean, and the proxied constructor returns 1950. Fixed by role plus a `Date.now`-only member allow-list; the widened probe catches **17 of 17** (v4.14). |
| **Minor 14 + the sweep** | The refuted *"can never disagree"* claim lived at **five** sites, not the four listed; all corrected to name the enforcing test. The ordering invariant is on **two** tables — `PYTHON_NONPRINTABLE_RANGES` is binary-searched too — so the shape test covers both. The UCD-drift row's comment went from 16 lines to 4. **Nit 5** closed. |
| ⚠️ **A rule-15 violation of my own, all phase** | I recorded *fixture ✅ at baseline* from `fixture-check.sh`'s **exit code** in every round. The script's own header says it only prints counts, sequences and `xmin` cardinality, and that *you diff the whole output against a saved run*. Its exit 0 meant "it ran". Measured properly this round: diff against the tester's persistent end-of-Phase-6 capture → identical. My first attempt at that diff hit a baseline file that no longer existed (`/tmp` does not survive a reboot) and returned diff's exit **2** — "could not compare", not "drift" — which is why the comparison now points at the capture under `~`. |
| **Standing rule — §4 rule 15** | *A universal claim must name the check that enforces it, or be written as a measurement.* The reviewer found that six of the seven claims behind rounds 2–6 were this one error with different nouns. **15b:** when you pin a table, assert its shape in the same commit. |
