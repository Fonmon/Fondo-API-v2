...
3a. ⚠️ **Drain the scheduler backlog before enabling the runner — P7-D1.** v2's D7 rule is
   ...
   **one is not** — task **1497**, a `YEARLY` birthdate task for owner **3** (Fernando,
   ...
   So **drain task 1497 by id** — the `AND repeat = 0` clause spares it, and this step un-spares
   it deliberately, for a stated reason rather than as a side effect of a predicate:
   ```sql
   UPDATE fondo_api_schedulertask SET processed = true WHERE id = 1497;  -- owner 3, is_active = f
   ```
...
3a-bis. ⚠️ **Neutralise task 2142 before 14 November 2026 — no drain predicate reaches it.**
   ...
   ```sql
   UPDATE fondo_api_schedulertask SET processed = true WHERE id = 2142;  -- owner 15, is_active = f
   ```
   ⚠️ **Marking it processed writes no clone, which is exactly what Q35 wants here** — it ends
   the chain, and that is the decision, not an accident. Re-run the `owner_id` sweep below at
   cutover rather than trusting these two ids: other departed members may have pending chains.
   ```sql
   SELECT t.id, t.run_date::date, t.repeat, t.payload->'owner_id' AS owner, u.is_active
   FROM fondo_api_schedulertask t
   JOIN auth_user u ON u.id = (t.payload->'owner_id')::int
   WHERE t.processed = false AND t.payload->'type' = 'birthdate' AND u.is_active = false;
   ```
...
| **D12** | CAP auto-close was never implemented — `services/saving_account.py` carries a `# TODO: schedule task for closing CAP`. Closing is manual-only today. | **Implement it** (Q20): a CAP closes automatically on `end_date`. New functionality, not a port. Needs a `SchedulerTask` type — **so Phase 6 depends on Phase 7** (already sequenced that way). No member notification (Q22). **The close writes `state = 1`; closed-ness is never derived at read time.** A reconciliation query over `state = 0 AND end_date < today` (America/Bogota) is the independent check — and the ship-day backfill (**C74/C78**). **The close task carries `repeat = 0`, which is what makes its executer's `throw`-on-failure safe** (**C77**). | P6 | ✅ **Decided — build** |
...
| C68 | **The run summary never reaches a log, and the runbook check cannot tell a completed pass from a dead one.** `handleCron` discards `run()`'s return value, so the promised `failedDelivery` count is computed and thrown away; a whole-pass failure goes to `@nestjs/schedule`'s default `console.error`, so `grep -c 'Running scheduler'` returns **2** whether the pass processed 110 rows or died on the next statement. **The phase wrote that grep as its own verification step** — C67's rule failing on the phase that introduced it. | P7b · major | 🟡 **Open → P6 gate.** Documented, not fixed: `SchedulerExecuterOutcome`'s docblock said `ok: false` is "warned **and counted**", which reads safer than it is; it now says the count reaches no log and points here (**F-a**). The fix is still this condition's. |
| C69 | **A failed `release()` swallows the executer's error and silently keeps the row `processed`.** If `release()` rejects, the `throw error` after it is unreachable: the executer's error is destroyed and the row stays claimed with nothing sent — a lost notification, filed as a database error. | P7b · major | 🟡 **Open → P6 gate** |
| C70 | **`SCHEDULER_ENABLED` fails closed on a typo.** `SCHEDULER_ENABLED=ture` is silently `false`, and the spec *pins* that leniency. Zero instances — nobody holding the flag, nothing erroring, reminders simply stopping — is the failure mode §7.2 names as the most likely cutover mistake, because it is the default. | P7b · major | 🟡 **Open → P6 gate** |
| C71 | **D7 has a forward-going birthday consequence no register row covers, and the drain does not fix it.** A birthdate set or corrected *after* the member's birthday writes an already-past-due task. Under v1's `=` it never fires; under D7's `<=` it fires on the next pass and pushes *"Hoy está cumpliendo años X"* to the whole roster **on the wrong day**. Backlog drainage does not touch it — every future correction recreates it. | P7b · major | 🟡 **→ `business-analyst`** |
...
| C73 | **F2 — `Sending request to MNS...` is neither ported nor registered.** `fondo_api/celery/tasks.py:15` logs it on every publish; v2 emits `Message sent, id: …` and the error line but not this one. `grep "Sending request to MNS" docs/*.md` returns nothing. Phase 2 origin, surfaced here because this phase is the line's main caller. | P7b · minor | 🟡 **Open → P6 gate.** `nestjs-reviewer`'s stated preference is to **port the line** — one line at the top of `publish()` — recorded here so Phase 6's gate does not re-derive it. |
...
| C75 | **Two unregistered log-stream differences.** v2 passes `error.stack` as `Logger.error`'s second argument, so every caught error emits a **stack block v1 never prints**; and the two new WARN lines (`skippedClaimed`, `failedDelivery`) have no v1 counterpart. Both additive, both improvements, neither touches a row or a byte — but the register's value is that it is complete. | P7b · minor | 🟡 **Open → P6 gate** |
| C76 | **No cell pins the UTC-space clone where the Bogotá day and the UTC day differ.** Every clone cell — unit, e2e and every parity ME cell — uses `05:00Z`, where the two dates agree, so nothing distinguishes v1's **UTC-field** arithmetic from a Bogotá-local implementation and a future "fix" to local-space would pass the whole suite. Suggested discriminating cell: `run_date = 2026-01-31T01:00:00.000Z`, `repeat = 3` → UTC-space `2026-02-28T01:00:00.000Z` vs Bogotá-local `2026-03-01T01:00:00.000Z`. **One day apart.** | P7b · nit | 🟡 **Open → P6 gate** |
| C77 | **The throw rule must be stated as a function of `repeat`, and D12's close task must be `repeat = 0`.** C74 restated **Q6** on the money/not-money axis; that is true for D12 and **incomplete as a rule**. Q6's decision was really about `repeat`: `ok: false` buys chain survival at the cost of one occurrence, a trade that only has a numerator when there *is* a chain. Verified mechanism: `scheduler.runner.ts` releases and rethrows **before** `createRepeatInstance`, so a throw skips the clone — give the close task a non-zero `repeat` and "must throw" reintroduces exactly the chain-breaking failure Q6 avoided. | P7b → P6 · raised by `nestjs-developer`, sustained by `nestjs-reviewer` | 🟡 **Open → P6 gate** — ✅ **doc clauses landed**: `SchedulerExecuterOutcome`'s `ok` docblock (both the `throw` bullet's missing clone consequence and the D12 carve-out's precondition), `docs/phase-7b-deviations.md` §7.4 **item 3** (a clause on item 3, not a fifth item — it is item 3's precondition and splitting it invites reading item 3 alone), §3 Phase 6 *Risks*, and **D12's Change column in §5**. **Gate obligations (words alone would repeat C74's failure one level up):** (i) `repeat = 0` written literally at D12's task-creation site; (ii) a unit cell asserting the inserted row's `repeat` is `0`; (iii) a cell asserting a **throwing** close executer leaves the row unprocessed **and writes no clone** — the only cell that catches someone "fixing" the runner to clone before running. |
| C78 | **C74's two remedies are not interchangeable; narrow it to the reconciliation query.** "Derive closed-ness from `end_date` on read" is not a design option with a cost — v2 **already ships** the port (`src/users/user.service.ts:389` aggregates `savingAccount` at `state: 0`), so it would mean editing a **Phase 3 path that has already passed its gate**, plus Phase 6's own `state` filter. Two further reasons from `nestjs-reviewer`: it creates **two sources of truth in a one-column state model** (what does a treasurer's `PUT state = 0` mean on a row whose `end_date` is past?), and it **defeats C74's purpose** — a derivation is not an independent check, it makes the runner's failure invisible rather than detectable, which is the zero-instance failure mode again. | P7b → P6 · raised by `nestjs-developer`, sustained and strengthened by `nestjs-reviewer` | 🟡 **Open → P6 gate** — ✅ **doc clauses landed**: `SchedulerExecuterOutcome`'s idempotency paragraph, `docs/phase-7b-deviations.md` §7.4 **item 4**, §3 Phase 6 *Risks*, and **D12's Change column in §5** — derive-on-read dropped from all four. **Ruling: closed-ness is materialised in `state`.** Close = `UPDATE … SET state = 1 WHERE id = ? AND state = 0` (idempotent, race-safe). Reconciliation = `SELECT id … WHERE state = 0 AND end_date < <today, America/Bogota>` — which **also answers the first open operator question on D12** (CAPs already past `end_date` on ship day: the same query is the backfill). Belongs in **D12's Change column in §5**, not only §3 *Risks* — by the precedence rule, a ruling that lives only in a Risks bullet is describing, not deciding. |
...
### ▶ Now — Phase 7b (Scheduler runner) **CLOSED**; Phase 6 (CAPs) unblocked.

| | |
|---|---|
| **Branch / head** | `feat/phase-7b-scheduler`, head **`a191f74`** (parent `cfa57ea`), branched from `d88e3af` |
| **Gate** | lint ✅ · `tsc` ✅ · **2173 unit / 67 suites** · **1061 e2e + 2 skipped / 20 suites** · `fondodev` at baseline on counts **and** sequences **and** `xmin` cardinality. **Every one asserted by exit code** (C67). |
| **Pipeline** | Phase 7b ✅ dev → ✅ **`manual-tester` PASS** (`3659eef`) → ✅ **`nestjs-reviewer` approved w/ conditions C68–C76** → ✅ **C74 closed** (`a191f74`) → ✅ **validated: `manual-tester` PASS on all four items, `nestjs-reviewer` APPROVE, `business-analyst` returned with concerns (all three escalated to the operator and answered — Q35, Q36, Q37)** |
| **Result** | ✅ **APPROVED WITH CONDITIONS — C68–C76.** **C74 closed** (`a191f74`), which was the only one gating Phase 6's start. **C72's code half closed** (`cfa57ea`). C68–C70, C73, C75, C76 roll to Phase 6's gate. |
| **In flight** | ✅ **C77 + C78 + F-a clauses landed** across `scheduler-executer.ts`, `docs/phase-7b-deviations.md` §7.4 (items 3–4) and §3 Phase 6 *Risks* — plus **D12's §5 Change column**, which is where the C78 ruling actually decides rather than describes. ⚠️ `nestjs-developer` hit the session rate limit **after** writing all the edits but **before** verifying or committing them; I reviewed the diff, re-ran the gate and committed. 🟡 **D39** still needs a layer decision in the phase that implements it. |
| **Next phase** | **Phase 6 — Saving accounts (CAPs).** Start is unblocked; C74 was the only gate. It inherits the zero-instance failure mode wholesale: no flag means CAPs silently stop closing exactly as reminders silently stop sending, and **C68/C70 are the detection story for both**. Carries **C77** (`repeat = 0` + three test cells) and **C78** (`state` materialised, reconciliation query) into its gate. |
| ⏰ **Dated deadline** | **Task 2142 fires 14 November 2026 — 68 days from 2026-09-07.** Angi Paola is `is_active = f`; under **Q35** it must not fire, and **no drain predicate reaches it** because it is future-dated. Runbook **step 3a-bis**. It fires under **v1 too** if cutover has not happened by then, so this deadline is independent of the migration. |
| **D12 decided** | ✅ Both operator questions answered **before** Phase 6 started (**Q38**, **Q39**). Ship-day backfill = C78's reconciliation query. Close runs on the **10:00 pass only, with no code** — date-granularity selection plus the claim already produce it; the 14:00 pass is the retry path C77 needs. |
| **Open operator questions** | Two, on **D12**, to ask when Phase 6 starts: what happens to a CAP whose `end_date` is already past on ship day, and whether the close runs at the 10:00 or the 14:00 Bogotá pass (or both). |
...
| 6 Saving accounts (CAPs) | 🟡 **NEXT — start unblocked** (C74 closed `a191f74`) | — | — | — | — | **C74 ✅** — D12 must `throw`, never `ok: false`, and must be idempotent/reconcilable (§3 Phase 6 *Risks*, `docs/phase-7b-deviations.md` §7.4 items 3–4). 2 operator questions open on **D12**; inherits **P7-D2**'s resolve-before-claim constraint. Also inherits **C77** (the close task must be `repeat = 0` — that is what makes "must `throw`" safe) and **C78** (closed-ness materialised in `state`; reconciliation query, never derive-on-read): words landed, **code obligations gate this phase** |
