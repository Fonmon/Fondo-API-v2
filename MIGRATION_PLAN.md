...
| C94 | **The second `migrate deploy` in `test/test-database.ts` was labelled stage 2b in four places**, while its own trigger says "when Release B's code lands" — which is stage 2a. A developer starting stage 2a would find every e2e suite failing at bootstrap (`fondo_api_test` still hstore) and the shortest visible way out is `git mv` of the step-6 directories into `prisma/migrations` — C92 defeated by the document meant to enforce it. | P9 · re-check N1 | 🟡 **Open** — split the bullet: the provisioner's second run belongs to **stage 2a**, in the same commit as the codec deletion; stage 2b is only the directory move and deleting the knob. |
| C95 | **Preflight check 7 cannot see an expression or predicate index** (`pg_attribute` joined on `a.attnum = ANY(i.indkey)`, and `indkey` holds `0` for an expression column). Measured by `nestjs-reviewer` on a scratch 17 clone: an index on `akeys(payload)` is invisible to the check and then **aborts the conversion** with `ERROR: current transaction is aborted` — the masked message the two-file split exists to prevent. `fondodev` has no such index and Django would not create one; production is unmeasured. | P9 · re-check N2 | 🟡 **Open** — replace the index branch with a `pg_depend` sweep (measured to return the offending index name where check 7 returned 0), keep the `pg_rewrite` branch, add an e2e cell. |
| C96 | **`PRISMA_MIGRATIONS_PATH` left in a task definition would convert production on a routine deploy** — and because Release A carries the step-6 directory through cutover by design, the deployed build's own guard would then refuse to start: the one-way door fires, the service is down and rollback to v1 is dead, from an unwatched deploy. Mitigated today by three runbook prohibitions, the resolved path printed on every invocation, and the guard's loud failure. `nestjs-reviewer` accepted the warn-on-non-default deviation as correct (warning on *set* would fire every e2e run). | P9 · re-check N5 | 🟡 **Open — close it rather than mitigate:** `resolveMigrationsPath()` throws unless a second variable carries the current date (`STEP6_CONFIRM=YYYY-MM-DD`), so a leftover goes stale within a day and the door needs two correct variables set on purpose. |
...
| 9 Cutover, hstore→jsonb | 🟡 **STAGE 1 APPROVED WITH CONDITIONS** (`4db69fb`) — B1–B3 and M1–M7 closed; **C94**, **C95**, **C96** to fix, then **stage 2a** (go given) | ✅ stage 1 | ⬜ | 🟡 Approved w/ conditions | ✅ `ba-phase-9-c49-redecisions.md`, `ba-phase-9-step6-questions.md` | C91 ✅ ruled; C92 ✅; **C93** stage 2a before cutover; **C94–C96** open; C39 answerable; C83, C90 cleanup |
