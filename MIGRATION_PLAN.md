...
| **Stage 1 landed (`de17e97`)** | No stop condition hit: `schedulertask.payload` 626 rows with one key set and `user_ids` 626/626 parsing; `subscription` 94 rows, `keys` 94/94 valid after v1's `'`→`"` repair and 0/94 containing a `"`; **0 duplicates** blocking D6/D11 (374/374, 15/15, 15/15). **C39 answerable:** Prisma 7 treats `0_init` as applied via `finished_at`, never reading `applied_steps_count` — `migrate deploy` is exit 0 in three measured shapes. Four migrations in `prisma/migrations-step6/` (preflight, conversion, two uniques); proof on a reset clone: entries, rows, key order, reverse and a corruption control all PASS. Measured: a Prisma migration file is **not** atomic by default, and `BEGIN/COMMIT` masks the real error — so stop conditions sit in an unwrapped preflight and the conversion is wrapped. |
| **Operator answers (Q56–Q59, 2026-09-15)** | 🔴 **Q56 — production deploys DO run migrations**, so the step-6 directory stays out of `prisma/migrations` until step 6 has run in production; the Release B sequence must assume an auto-migrating deploy. **Q57** — Release A gets the mirror boot guard. **Q58** — leave the three redundant Django indexes. **Q59 — production is PostgreSQL 17.x**; re-prove the migrations there (the proof ran on 18.6). |
| **`nestjs-reviewer` (Phase 9 stage 1)** | 🟡 **APPROVED WITH CONDITIONS** on the fix round `4db69fb`; **stage 2a is GO**. All three blockers and all seven majors closed. It verified on the **17.11** migrated clone rather than from the report: both columns `jsonb`, 626 / 94 rows, **3411** entries (= 626×5 + 94×3 − 1), 0 still doubly-stringified, `owner_id` string ×626, 94/94 `keys` flipped to `auth,p256dh` (C91 confirmed), 3 unique constraints, xmin cardinality 1/1, and `0_init` still at 0 with four step-6 rows applied — so D34/C39 holds on 17 as well as 18. It called the mutation table, the four isolated controls, the fail-closed guard cells and the 17.11 re-proof *the strongest evidence any phase in this migration has produced*. **Conditions:** **C94** a stage-2a/2b label that would push the directory move; **C95** an expression index is invisible to preflight check 7 and then aborts the conversion behind the masked error (measured); plus N3/N4 script nits and **C96**, which I am closing rather than mitigating. |
| **`nestjs-reviewer` (Phase 9 stage 2a)** | **`5d019d9` APPROVED; `204aa88` APPROVED WITH CONDITIONS; cutover NO-GO until C97.** Re-measured rather than read: `migrate diff` of `schema.prisma` against the migrated 17 clone is **empty** and against the un-migrated clone is exactly the five expected statements; the PG18-vs-17 `NOT NULL` catalogue difference reproduces; a tree-wide sweep finds **no bare `->` comparison left** (every survivor is inside `jsonb_typeof`, a deliberate jsonb read, or hstore-era preflight SQL); only the two flagged raw queries remain in `src/`; `hstore-legacy.ts` is **byte-identical** to the deleted codec's five functions; C91's pin sits on the one production path every SQS body takes. It accepted both judgement calls — the two raw zone-comparing queries stay (rewriting the fund's only reminder predicate on the day of a one-way door buys nothing step 6 needed), and `django-str.ts` surviving the codec is *the strongest reasoning in the commit*, because 626 rows hold `"owner_id": "9"` and the Phase 7 dedupe compares it as text. |
| **Release A tagged and gated (C97 closed)** | **`release-a-cutover` = `5d019d9`** — the build cutover step 3 deploys, now named there and in the recovery path, and recorded as frozen. I gated the tag myself: lint 0, `tsc` 0, **2619 / 82** unit, **1386 + 2 skipped** e2e on an `hstore` test database, fixture diff 0 / control 1. ⚠️ The first attempt failed 1306 cells because `fondo_api_test` still held head's **converted** schema and Release A's guard refused to start, naming both columns — the guard working, not a defect. **Gating either build requires dropping `fondo_api_test` first.** |
| **`business-analyst` + operator (Q60–Q62)** | **C91 ruled: preserve `p256dh, auth`** in Release B (Q60) — measured 94/94 in that order, no signature/hash/FIFO anywhere on the SQS path, but the consumer is out of repo, so the operator took the no-change option. **Q61:** nothing outside the API reads those two tables, so step 6 touches only v1 and v2. **Q62:** the PWA subscribes only on explicit opt-in, so deleting a subscription row silently ends that member's push — the runbook must say so. Also confirmed: `owner_id` stays a **string** (it never reaches the SQS body; it is overloaded — a member id for birthdays, a **loan** id for reminders), and the preflight's stop on a `keys` containing a `"` is right. |
...
| 9 Cutover, hstore→jsonb | 🟡 **STAGE 2a ACCEPTED; Release A tagged and gated** (`release-a-cutover` = `5d019d9`). Remaining: **C98**, **C99** docs; then cutover is go, step 6 operator-run, stage 2b after | ✅ stage 2a | ⬜ | 🟡 Approved w/ conditions | ✅ two BA notes | C91–C97 ✅; **C98**, **C99** open (docs); stage 2b blocked on step 6 (correct) |
