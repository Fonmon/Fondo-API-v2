...
### ▶ Now — Phase 6: fix round landed, awaiting `nestjs-reviewer` re-check.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 at `cda69e0`, plan at `2f74751`, fix round on top |
| **Pipeline** | ✅ dev → ✅ `manual-tester` **PASS with 3 failures** → 🔴 `nestjs-reviewer` **CHANGES REQUESTED** → ✅ fixes landed → ⬜ **re-check** |
| **Why it was blocked** | Not for anything Phase 6 wrote — the reviewer's words were *"the port is faithful and D12 is the strongest new-functionality design this migration has produced"*. Three defects in **Phase 0 shared helpers** that reproduce on this phase's routes, one of them writing a wrong row silently. |
| **B1 ✅** | `pythonInt`/`toDjangoInt` refused Unicode `Nd` digits and PEP 515 underscores. ⚠️ **Not fixed with `\p{Nd}`** — CPython 3.9.25 is UCD 13.0.0 (650 code points), Node 24 is UCD 17.0 (770), so a property test would have **accepted 120 code points v1 refuses**. Table captured from the pinned interpreter. Controlled 3 ways: no-fold **6** failures, no-PEP-515 **6**, `\p{Nd}` **9**, restored clean. |
| **B2 ✅** | `Date.UTC` remaps years 0–99 to `1900 + year`, at **four** sites. One `utcMillisFromParts` helper + `1 ≤ year ≤ 9999` in `toDjangoDate` + an era guard in `partsInZone`. |
| **B3 ✅** | ⚠️ **The tester's stated cause was wrong and the reviewer caught it.** Not PostgreSQL range — `zonedTimeToUtcMillis` remapped the *Bogotá-local* year 99 to 1999, making the offset correction −1900 years and producing year **−1800**. Controls prove it: mutating the other two sites fails **1** cell each and **never** the `0100-01-01` cell; mutating this one fails **2**, including it. **Fixing the obvious sites alone would have looked green.** |
| **Gate** | lint ✅ · `tsc` ✅ · **2304 unit / 70** (was 2274) · **1170 e2e + 2 skipped / 21** (was 1160+2). ⚠️ **Lint failed on the first run and I caught it by exit code** — the false-green #21 countermeasure doing its job. |
| **Docs ✅** | **D41, D42, D43** registered (parse-side integer precision — seen in Phase 5 as L2, seen again as P6-F5, registered neither time); **D37** fourth site; **P6-F5** split render/parse; **C77(iii)**'s falsified sentence moved onto the cell where it is true; `scripts/parity/reset-clone.sh` committed 755; false-green **#23**. |
| ⚠️ **My error, corrected** | I wrote *"both runbook sites corrected"* for C68. **There were three** — and `docs/phase-6-deviations.md` §7.1 had named the third explicitly. It is the site an operator has open at cutover. *"I corrected the sites I found" is not "I corrected the sites that exist".* |
| ⏰ **Dated deadline** | **Task 2142 fires 14 November 2026** — Angi Paola, `is_active = f`; under **Q35** it must not fire and no drain predicate reaches it. Runbook **step 3a-bis**. |
...
| 6 Saving accounts (CAPs) | 🟡 **Fix round landed; awaiting `nestjs-reviewer` re-check** | ✅ | ✅ **PASS w/ 3 failures** (all in Phase 0 helpers, all fixed) | 🔴 **CHANGES REQUESTED** → fixes landed, re-check pending | ✅ Q19–Q24, Q38, Q39 | **C68–C70, C73, C75, C76 ✅**; **C77, C78 code ✅**; **B1, B2, B3 ✅ fixed** w/ mutation controls; **D40–D43 ✅ registered**; false-green **#23** ✅; six deviations **P6-F1**–**P6-F6** |
