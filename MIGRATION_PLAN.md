...
| C71 | **D7 has a forward-going birthday consequence no register row covers, and the drain does not fix it.** A birthdate set or corrected *after* the member's birthday writes an already-past-due task. Under v1's `=` it never fires; under D7's `<=` it fires on the next pass and pushes *"Hoy está cumpliendo años X"* to the whole roster **on the wrong day**. Backlog drainage does not touch it — every future correction recreates it. | P7b · major | 🟡 **Decided (Q47) — implementation Phase 8b** as **D48**. Folds in Q36: once D48 lands, saving Ainhoa's profile writes 2027-08-05 and her chain starts correctly, with no manual insert. |
...
### ▶ Now — Phase 8b fix round landed; `nestjs-reviewer` re-check running.

| | |
|---|---|
| **Phase 8 — Files + admin** | ✅ **CLOSED — approved.** `nestjs-reviewer` approved the fix round at `8948f79` (M1 D22 removed from `POST /api/file`, m1 `Vary: Origin` pinned, m2 measured list). Gate on `8948f79` re-measured by me, each step's own exit code: lint 0, `tsc` 0, 2513 / 77 unit (−4, all in `drf-request-data.spec.ts`), 1331 + 2 e2e (every suite ±0), fixture diff 0 / control 1. `manual-tester` re-measured the non-ASCII boundary: 500/500. |
| **C86 — `X-Frame-Options`** | Found in the re-measure, ruled register-only; pinned at `a963e5f` (lint 0 · `tsc` 0 · **2513 unit / 77** · **1331 e2e passed + 2 skipped**; control removing the header fails 7 cells). D22/D24 wording widened. |
| **Phase 8b — Birthday notifications** | 🟡 **Implemented** at `5e3457a`: **D48** next birthday not yet missed (a birthday today keeps today only strictly before 14:00:00.000 Bogotá), **D39** a departed or missing owner sends nothing while the row is processed and cloned, **D49** recipients = active members except the owner at send time (stored `user_ids` still written and parsed, no longer the audience). Gate re-measured by me on `5e3457a`, each step's own exit code: lint 0, `tsc` 0, **2573 / 79 unit** (+60: executer +24, member-directory +7 new, runner +1, host-zone +3 new, user.service +25), **1345 + 2 e2e** (+14: scheduler-runner +8, user +6), fixture diff 0 / control 1. Mutation: 20 of 20 compiling behaviour mutants killed, 3 of 3 blind controls survived, 4 non-compiling plants superseded by killed re-plants; all 14 committed target hashes match `docs/phase-8b-deviations.md` §5.1 (checked). **Next:** `business-analyst` (alignment + Q-8b-1..4) and `manual-tester`, then `nestjs-reviewer`. |
| **`business-analyst` (Phase 8b)** | **Code aligned** with Q47–Q49 on all six registered choices; "active members" = `is_active` with no role filter, same as v1 (all 13 active users, every role). **Q-8b-1** accept a save committing just after the 14:00 pass (a day-late greeting), no margin. **Q-8b-2** keep the loud failure — but it does not end the chain (**C87**). **Q-8b-3** keep the yearly WARN; the runbook sweep now sees missing owners. **Residual, v1 behaves the same:** a profile save carrying `birthdate` between 10:00 and 14:00 on the member's birthday deletes the already-sent row and D48 schedules today again — the fund is greeted twice. |
| **Operator decisions (2026-09-14)** | **Q51** — delete tasks 1497 and 2142 (runbook 3a / 3a-bis rewritten; sweep corrected and measured). **Q52** — how a member returns is not established. |
| **`manual-tester` (Phase 8b)** | ✅ **PASS** on `5e3457a` (built from `git archive`). **Write side** (`PATCH /api/user`, after 14:00 Bogotá): past and today-after-14:00 birthdates → v2 next year, v1 this year (D48); future equal; 29 Feb → v1 500, v2 2027-02-28 (D19 × D48); Ainhoa → v1 **409** (D15: her email is user 7's username), v2 2027-08-05 — so **Q36 holds only in v2**. **Send side** (one pass per executer on its own clone, capture stubs): inactive and missing owner → v1 publishes, v2 nothing (D39); stale list → v1 68 subscriptions, v2 90 = active members minus owner (D49); sole active owner → v2 nothing; **payment reminders byte-identical**; resulting rows and successors byte-identical. `fondodev` schedulertask unchanged (count, max id, xmin, content md5); fixture diff 0 / control 1. Harness note: `ANALYZE` clones after a reset before comparing unordered lists. |
| **`nestjs-reviewer` (Phase 8b)** | 🟡 **APPROVED WITH CONDITIONS** on `5e3457a`. Verified: the single Bogotá reading and 14:00 boundary, D19 on the chosen year, the host-zone spec's zone check, the executer's check order against v1/C23, one shared active-members query with no role filter, one `Clock` provider, re-pins stricter not weaker, B4 correctly labelled. **Conditions:** **M1 / C88** int4 `owner_id` throws (measured P2020) — guard it; **m1 / C87** wrong "ends the chain" wording at three sites; **m2 / C89** single zone source; **m3** record the non-atomic owner/audience reads as harmless; **n1** `§2.4` → `§2.5`; **n2** two rule-15 phrasings. Then mutation re-run on changed targets and the gate. |
| **Fix round (`796ba2b`)** | **C88** — an `owner_id` above int4 now skips as missing with no query (unit boundary 2147483647 / 2147483648; e2e `99999999999` on the real table). **C89** — `TIME_ZONE` pinned to `America/Bogota` in the env schema, so D48, the runner's selection, the repository's dedupe and the cron share one zone; any other value fails at boot (v1 hardcodes the same zone). **C87** — "stalls, not ends" at all three sites. m3/n1/n2 done. Gate re-measured by me: lint 0, `tsc` 0, **2590 / 80 unit** (+17: executer +3, zone-single-source +12 new, host-zone +2), **1348 + 2 e2e** (+3, all C88), fixture diff 0 / control 1; all 17 hashed files in the register match the commit. Mutation (developer): 27/27 behaviour mutants killed, 4/4 blind controls survived, every existing mutant re-run. |
| **Open before cutover** | **C84** — someone with bucket access compares `fonmon` against the live `File` rows. **C83** — path-id hoist, Phase 9 cleanup. |
...
| 8b Birthday notifications | 🟡 **FIX ROUND LANDED** — `796ba2b`; gate re-measured independently; awaiting `nestjs-reviewer` re-check | ✅ | ✅ **PASS** (`parity-phase-8b.md`) | 🟡 Approved w/ conditions → re-check | ✅ `ba-phase-8b-birthdays.md` — concerns resolved by **Q51**/**Q52** | **C71**, **D39**, **D48**, **D49**; **C87**, **C88**, **C89** fixed at `796ba2b`, pending re-check; runbook 3a/3a-bis delete 1497/2142; task 2142 deadline 14 November 2026 |
