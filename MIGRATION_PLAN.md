...
### ▶ Now — Phase 8 parity PASS; awaiting `nestjs-reviewer`.

| | |
|---|---|
| **Phase 8 — Files + admin** | ✅ **Parity round PASS** on `5f58b11`: no unexpected difference between v1 and v2 on files, admin, or the Phase 3 and 4 bulk uploads (`docs/parity-phase-8.md`). **Next:** `nestjs-reviewer`. |
| **What the round measured** | 68 read-only cases (every difference pre-declared); 83 `POST /api/file` cases (D47 19, D46 5, D37 id offsets 15, the rest pre-declared or their direct consequences); **D46 and D47 conform to the spec**, the `Dup X` refusal included, with zero storage calls on every refusal; the Q45 race reproduced as the known residual; **C81 closed**; admin sends only to the caller; signed URLs are v4 with a 300-second expiry. Gate re-measured by exit code: 2517 / 77 unit, 1331 + 2 e2e (22 + 1 of 23 suites). `fondodev` baseline diff 0 with control 1, at start and end. |
| **Two corrections to the developer's doc, made here** | The expected-diffs table said headers match on init-time multipart 500s; they do not (v1 no `Vary`, v2 `Vary: Origin`) — now **C85**. And a trailing-space boundary is 500/500 only when **quoted**; unquoted, both stacks strip the space and succeed 201/201. |
| ⚠️ **A gap in the round, recorded rather than passed** | v1 was given throwaway `authorized_user` credentials and a proxy on a closed port, but no control showed that proxy actually blocks traffic. No real credentials exist on this host (measured), so nothing authenticated could have reached `fonmon`; the fence itself is still unmeasured. |
| **Open for the reviewer** | **C79** — D22 on `POST /api/file`. **C80 / D45** — U+2028/U+2029 escaping in all JSON. **C85** — `Vary: Origin` on init-time multipart 500s. **C83** — path-id hoist (non-gating). **C84** — before cutover, compare `fonmon` against the live file rows (operator action). |
...
| 8 Files + admin | 🟡 **Parity PASS** — no unexpected difference (`docs/parity-phase-8.md`); awaiting `nestjs-reviewer` | ✅ | ✅ **PASS** | ⬜ | ✅ `ba-phase-8-files.md` | C81, C82 ✅ closed; C79, C80, C83, C84, C85 open |
