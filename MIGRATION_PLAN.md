...
### ▶ Now — Phase 6: second fix round landed; awaiting `nestjs-reviewer` re-check #2.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 `cda69e0`, fix round 1 `6a859d0`, §5 rows `aa2fa56`, fix round 2 on top |
| **Pipeline** | ✅ dev → ✅ tester **PASS w/ 3 failures** → 🔴 reviewer **CHANGES REQUESTED** → ✅ fix 1 → 🔴 **CHANGES REQUESTED again** → ✅ fix 2 → ⬜ **re-check #2** |
| **Gate** | lint ✅ · `tsc` ✅ · **2309 unit / 70** (was 2304) · **1174 e2e + 2 skipped / 21** (was 1170) · fixture ✅. Every one by exit code. |
| 🔴 **The blocker, and it was mine** | **B2 was fixed at four sites and live at four more** — `parseBirthdate` and `strptimeIsoDate` **reachable** (both **v1 200 / v2 500** for a two-digit year: each builds a `Date.UTC` probe and compares `getUTCFullYear()` back, which the remap makes fail), plus two latent in `relativedelta.util.ts`. ⚠️ **`date.util.ts` asserted “every `Date.UTC` call must go through here” while that was false** — the **M2 lesson recurring in code four hours after I wrote it down in the docs.** |
| **The fix that is not prose** | An **ESLint `no-restricted-syntax` rule** bans `Date.UTC` anywhere in `src/` outside `date.util.ts` (specs excepted). **Controlled** — a fresh raw `Date.UTC` in `src/` reports exactly one error. Prose invariants go stale silently; this one cannot. |
| ✅ **Where I corrected the reviewer** | Its **Major 2** said all three date parsers need the Unicode-digit fold. **Measured on the pinned 3.9.25:** Django `date_re` + `int()` **accepts** `٢٠١٨-٠١-٠١` → `date(2018,1,1)`, but `strptime` **raises**. So the fold is `toDjangoDate`'s **only** — folding in the two `strptime` ports would have *created* a divergence. Both directions pinned. |
| ⚠️ **Two claims of mine were wrong** | **(a)** My `\p{Nd}` control count of **9** was unreproducible — my mutant returned `0` for *every* `Nd` including ASCII, conflating the fold axis with the digit-set axis. The faithful mutant gives **4**; the plan now **names the mutant**, because a count without one is not evidence. **(b)** My `reset-clone.sh` guard **could not fire** — `grep -q` writes nothing, so the piped `grep -v` always exited 1. Measured silent. **A check that cannot fail, shipped as the countermeasure for a false-green about checks that cannot fail.** Replaced with an `hstore` OID comparison across the restore. |
| **Caught by how I asserted** | My first draft of the route cells put `birthdate` at the top level; v1 reads `obj['personal']`. The route answered **200 and wrote nothing** — a cell that would have passed for the wrong reason had I asserted the status. Asserting the **row** caught it; the reason is now written beside the cells. |
| **Also corrected** | **D43** attribution (`toDjangoDate` is **P4** `0a8cf62`, not P0 — traced); **D42** gains the Unicode-`\d` axis; v4.7's clause annotated **inline** rather than rewritten; the lint-run inconsistency resolved; the generator's one-directional assert softened; `daysInMonth(0, 2)` documented as **still 28**, so the routing is not misread as having fixed year 0. |
