...
### ▶ Now — Phase 6: fifth fix round landed; awaiting `nestjs-reviewer` re-check #5.

| | |
|---|---|
| **Branch / head** | `feat/phase-6-caps` — Phase 6 `cda69e0`; fixes `6a859d0`, `aa2fa56`, `886cbf5`, `b035db3`, `3e6cb93`, round 5 on top |
| **Pipeline** | ✅ dev → ✅ tester → 🔴 CR → ✅ → 🔴 CR → ✅ → 🔴 CR → ✅ → 🔴 CR → ✅ → 🔴 **CR (Major 8)** → ✅ fix 5 → ⬜ **re-check #5** |
| **Gate** | lint ✅ · `tsc` ✅ · **2328 unit / 70** · **1177 e2e + 2 skipped / 21** (was 1175) · fixture ✅. Every one by exit code. |
| 🔴 **Major 8 — I demoted a WORKING control** | Last round I added the UCD-drift cell, found it did not discriminate, and labelled it *a regression pin, not a control*. ⚠️ **Wrong.** My mutant desynchronised the class from the fold — a state the code makes **impossible**, since both derive from the same array (verified over all 1,114,112 code points: 0 either direction). The reachable mutations, re-measured by me: **narrowing the class → 5 failures**; **the pinned table going stale → 3 failures, my row among them**; widening alone → unreachable. **The row is a control.** ⚠️ **And I had already measured half of this myself** — I ran the narrowing mutant earlier the same day, recorded *5 failures*, then drew the opposite conclusion from a different mutant without reconciling the two. My comment then told a maintainer the class was unobservable, which is an invitation to delete it or *simplify* it to the property test D42 forbids. |
| **Major 8 — a second self-inflicted near-miss** | My first table-drift mutant **passed 2328/2328 and proved nothing**: I inserted the new range out of ascending order, breaking the invariant the binary search depends on, so the entry was unreachable. Caught it, re-sorted, got the 3 failures. A mutant that cannot be reached is the same error as a check that cannot fail. |
| 🟠 **Minor 11 — the lint rule, third pass** | Nine more forms planted, **nine passed clean**. Eight new selectors close 8 of 9; the ninth (`(Date as any)['U'+'TC']`) survives. ⚠️ **The framing was the real finding**: *8 of 8 caught* reads as class closure and is closure over the plant set I chose. The config now states the boundary — **AST selectors cannot close value-flow aliasing**; once `Date` is bound to a parameter or field no syntactic pattern reaches it. The rule raises the cost of the accident; it does not make the remap unreachable. |
| **Also closed** | **Minor 12** — D44 had no route-level cell while its sibling D43 did; two added, one pinning that `POST` with `2020-02-30` is **500 with the table unchanged**. **Minor 13** — the docblock claimed *the row AND the notification* while the cell asserted only the row, **in the commit that fixed that exact pattern**; both asserted now. **Nit 4** — the `ValidationError` branch is pinned rather than any error. |
