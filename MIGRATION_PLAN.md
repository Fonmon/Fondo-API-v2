...
**§5 rows this phase owns:** **none of its own** — but the phase *discovered* **D35** (a live Phase 3/4 `null`-folding defect) and registered **P5-D1–P5-D3** locally. "Owns none" was determined mechanically and held; it did not mean nothing would be found. Determined mechanically, not assumed: the register's `Phase` column contains **no `P5` entry** (`grep -c '| P5 |'` → 0; the column reads P0 ×1, P1 ×1, P3 ×9, P4 ×10, P6 ×2, P7 ×1, plus the four cross-cutting rows D21–D24, which apply here as everywhere). A phase brief for Phase 5 that cites a §5 row is citing something that does not exist.
